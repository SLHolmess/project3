        ...
        stage("update docker tag") {
            steps {
                    ...
                    sh """
                        ...
                        sed -i "s#$IMAGE.*#${IMAGE}:${DOCKER_TAG}#g" k8s-config/orders/orders-deployment.yaml && \
                        cat k8s-config/orders/orders-deployment.yaml && cat services/orders/app/orders.js && git status && git add . && git commit -m "update tag: ${DOCKER_TAG}" && git config --list && \
